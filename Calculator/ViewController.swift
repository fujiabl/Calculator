...
class ViewController: UIViewController {
    ...
    let xButtonCount = 4 //１行に配列するボタンの数
    let yButtonCount = 4 //１列に配列するボタンの数
    var number1:NSDecimalNumber = 0.0 //入力数値を格納する変数1
    var number2:NSDecimalNumber = 0.0 //入力数値を格納する変数2
    var result:NSDecimalNumber = 0.0 //入力結果を格納する変数
    var operatorId:String = "" //演算を格納する変数
    ...
    override func viewDidLoad() {
        ...
        //計算ラベルのフレームを設定。横幅は端末の画面サイズから左右の隙間20ポイントひく
        resultLabel.frame = CGRect(x: 10, y: 30, width: screenWidth - 20, height: resultArea - 30)
        //計算結果ラベルの背景色を灰色にする
        //resultLabel.backgroundColor = UIColor.grayColor()
        resultLabel.backgroundColor = self.colorWithRGBHex(00000, alpha: 1.0)
        //計算結果ラベルをViewControllerクラスのviewに設置
        self.view.addSubview(resultLabel)
        //計算結果ラベルのフォントとサイズを設定する
        resultLabel.font = UIFont(name: "futura", size: 500)
        //計算結果らべるのアライメントを右揃えに設定
        ...
        // 計算結果フォントの色
        resultLabel.textColor = self.colorWithRGBHex(0xFFFFFF, alpha: 1.0)
        //計算ラベルをViewControllerクラスのViewに設定
        self.view.addSubview(resultLabel)
        // ボタンのラベルタイトルを配列で用意
        let buttonLabels = [
            "7","8","9","×",
        ...
        for y in 0 ..< yButtonCount {
            for x in 0 ..< xButtonCount {
                ...
                let arrayColors = [ colorWithRGBHex(0xFFFFFF,alpha:1.0).cgColor as AnyObject,
